...
{
	class CountdownType : CommandType
	{
		public override List<string> CommandNames() { return new List<string>() { "countdown" }; }
		public override string Help(string commandName) { return "Type command followed a number of minutes or amount of time formatted like 00:00:00."; }
		...
		class Countdown : CommandHandler
		{
			...
			public Countdown(SlackBotV3 bot) : base(bot) { }
			...
			private bool WaitForTimePeriod(double totalTimeToWait, SlackBotCommand command)
			{
				...
				System.Threading.Thread.Sleep((int)SECOND_IN_MILLISECONDS);
			...
			public override bool Execute(SlackBotCommand command)
			{
				...
				SlackBot.Reply(command, _message);
				if (initializeFailed) return true;
